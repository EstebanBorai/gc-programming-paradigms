class StringProcessor(object):
  ...
  def tokenize(self):
    """Return a TokenManipulator object and pass it a list of tokens from our string"""
...
class TokenManipulator(object):
  def __init__(self, tokens):
    """Create the TokenManipulator object. A list of tokens is required"""
    self.tokens = tokens
  ...
  def match_first_character(self, match_character):
    words_beginning_character = []

    for word in self.tokens:
      if word[0] == match_character:
        words_beginning_character.append(word)

    return words_beginning_character
